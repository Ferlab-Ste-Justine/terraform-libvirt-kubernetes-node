...
module "nfs_client_configs" {
  source = "git::https://github.com/Ferlab-Ste-Justine/terraform-cloudinit-templates.git//nfs-client?ref=v0.50.3"
  install_dependencies = var.install_dependencies
  proxy      = {
    client_name     = var.name
    nameserver_ips  = var.nfs_tunnel.nameserver_ips
    max_connections = var.nfs_tunnel.max_connections
    idle_timeout    = var.nfs_tunnel.idle_timeout
  }
  nfs_server = {
    domain = var.nfs_tunnel.server_domain
    port   = var.nfs_tunnel.server_port
  }
  tls        = {
    client_cert = var.nfs_tunnel.client_certificate
    client_key  = var.nfs_tunnel.client_key
...
module "kubernetes_node_configs" {
  source                      = "git::https://github.com/Ferlab-Ste-Justine/terraform-cloudinit-templates.git//kubernetes-node?ref=v0.50.3"
  install_dependencies        = var.install_dependencies
...
locals {
  cloudinit_templates = concat([
    ...
    var.nfs_tunnel.enabled ? [{
      filename     = "nfs_client.cfg"
      content_type = "text/cloud-config"
      content      = module.nfs_client_configs.configuration
    }] : [],
    var.chrony.enabled ? [{
      filename     = "chrony.cfg"
      content_type = "text/cloud-config"
